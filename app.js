...
// dDAE_1.017 — iOS BFCache: rebind tappable Home icons
try{
...
// dDAE_1.017 — error overlay: evita blocchi silenziosi su iPhone PWA
...
/* dDAE_1.017 — iOS hardening: Home icons always tappable (fallback binding) */
...
/* dDAE_1.017 — Tap counters: Adulti / Bambini <10 (tap increment, long press 0.5s = reset) */
...
// dDAE_1.017 — Fix contrast icone topbar: se un tasto appare bianco su iOS, l'icona bianca diventa invisibile.
...
if (page === "orepulizia") { initOrePuliziaPage().catch(e=>toast(e.message)); }
  ...
  // dDAE_1.017: fallback visualizzazione Pulizie
  try{
    if (page === "pulizie"){
...
async function loadOspiti({ from="", to="", force=false } = {}){
  ...
  const anno = (state && state.exerciseYear) ? String(state.exerciseYear) : String(loadExerciseYear());
  const lsKey = `ospiti|${uid}|${anno}|${from}|${to}`;
  const hit = __lsGet(lsKey);
  if (hit && Array.isArray(hit.data) && hit.data.length){
    const cachedRaw = hit.data;
    const cached = __filterByExerciseYear__(cachedRaw, anno, ["check_in", "checkIn", "arrivo", "dataArrivo", "check_out", "checkOut", "partenza", "dataPartenza", "createdAt", "created_at", "updatedAt", "updated_at"]);
    state.guests = cached;
    // Mantieni cache già filtrata (evita contaminazioni cross-anno/account)
    try{ __lsSet(lsKey, state.guests); }catch(_{}){}
    // render subito (perceived speed) solo se siamo nella lista ospiti
    if (state.page === "ospiti") {
  ...
  const applyNext = (data) => {
    const uidNow = (state && state.session && state.session.user_id) ? String(state.session.user_id) : "";
    const annoNow = (state && state.exerciseYear) ? String(state.exerciseYear) : String(loadExerciseYear());
    // Se nel frattempo è cambiato account/anno, ignora
    if (uidNow !== uid || annoNow !== anno) return;

    const nextRaw = Array.isArray(data) ? data : [];
    const next = __filterByExerciseYear__(nextRaw, annoNow, ["check_in", "checkIn", "arrivo", "dataArrivo", "check_out", "checkOut", "partenza", "dataPartenza", "createdAt", "created_at", "updatedAt", "updated_at"]);
    ...
    // Re-render dove serve
    try{
      if (state.page === "ospiti") {
        try{ requestAnimationFrame(renderGuestCards); }catch(_{}){ renderGuestCards(); }
      } else if (state.page === "statgen") {
        renderStatGen();
      } else if (state.page === "statprenotazioni") {
        renderStatPrenotazioni();
      } else if (state.page === "statmensili") {
        renderStatMensili();
      }
    }catch(_{}){}
  };
...
function computeStatGen(){
  ...
  // dDAE_1.017+ — Giacenza in cassa = (con ricevuta + senza ricevuta) - spese totali
  try{
...
// ===== dDAE_1.017 — Multi prenotazioni per stesso nome =====
  ...
  if (hdActions && !hdActions.__bound){
    ...
    hdActions.addEventListener("click", async (e) => {
      ...
      if (btn.hasAttribute("data-guest-del")){
        ...
        // ✅ dDAE_1.017: dopo cancellazione, vai SUBITO alla guest list (UX immediata su iOS)
...
// periodo iniziale
  ...
  // dDAE_1.017: salvataggio PULIZIE per-stanza (evita generazione righe/report inutili)
...
/* dDAE_1.017 — Calendario: blocca SOLO la colonna numeri stanze durante lo scroll orizzontale (fix iOS) */
...
function __fitCalendarioMonthLandscape(){
  try{
    ...
    // dDAE_1.017: in vista mese su iPad landscape usa tutta la larghezza disponibile (margine 10px L/R)
...
// --- FIX dDAE_1.017: renderSpese allineato al backend ---
...
// --- FIX dDAE_1.017: delete reale ospiti ---
...
// --- FIX dDAE_1.017: mostra nome ospite ---
(function(){
  const orig = window.renderOspiti;
  if (!orig) return;
  window.renderOspiti = function(){
