...
async function __fbExportAdmin__(opts){
  ...
  const datasets = await __readTablesSnapshot__(tables);
...
try{ if(!opts?.silent) toast("Operazione completata", "green"); }catch(_){}
  ...
  if (!opts?.skipReload){ try{ await __refreshAfterDataSync__(__restoreAfterSync); }catch(_){ } }
  return true;
}
...
async function __fbExportOperator__(opts){
  ...
  const datasets = await __readTablesSnapshot__([
    'pulizie',
    'operatori',
    'lavanderia',
    'colazione',
...
async function __fbImportAdmin__(opts){
  ...
  const __operatorDocs = await Promise.all((ops || []).map(async (op) => {
    try{
      ...
      if (!payload || !payload.datasets) return null;
      return payload;
    }catch(_){ return null; }
  }));

  for (const payload of __operatorDocs){
